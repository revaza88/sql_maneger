...
export class UserDatabaseService {
  ...
  /**
   * Create a new database using admin credentials
   */
  static async createDatabase(userId: number, databaseName: string, collation?: string): Promise<void> {
    ...
    // Use admin credentials to create the database
    const adminPool = new sql.ConnectionPool({
      user: config.database.user,
      password: config.database.password,
      server: config.database.server,
    ...
    try {
      ...
      await adminPool.request().query(query);
  ...
  /**
   * Delete a database using admin credentials (only if owned by the user)
   */
  static async deleteDatabase(userId: number, databaseName: string): Promise<void> {
    ...
    // Use admin credentials to delete the database
    const adminPool = new sql.ConnectionPool({
      user: config.database.user,
      password: config.database.password,
      server: config.database.server,
