...
class UserDatabaseService {
     ...
     */
    static async getUserConnectionPool(userId) {
        ...
        const userPool = new mssql_1.default.ConnectionPool({
    ...
    static async getUserDatabases(userId) {
        try {
            ...
            const ownedDatabases = await user_database_model_1.UserDatabaseModel.getUserDatabases(userId);
            ...
            const adminPool = new mssql_1.default.ConnectionPool({
     ...
     */
    static async createDatabase(userId, databaseName, collation) {
        ...
        try {
            ...
            const alreadyOwns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, sanitizedName);
            if (alreadyOwns) {
                throw new Error('You already own a database with this name');
            }
            // Create the database
            const collationClause = collation ? `COLLATE ${collation}` : '';
            const query = `CREATE DATABASE [${sanitizedName}] ${collationClause}`;
            ...
            await user_database_model_1.UserDatabaseModel.createUserDatabase(userId, sanitizedName);
     ...
     */
    static async deleteDatabase(userId, databaseName) {
        // Check if user owns this database
        const owns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, databaseName);
        if (!owns) {
        ...
        try {
            ...
            await user_database_model_1.UserDatabaseModel.removeUserDatabase(userId, sanitizedName);
     ...
     */
    static async getDatabaseTables(userId, databaseName) {
        // Check if user owns this database
        const owns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, databaseName);
        if (!owns) {
     ...
     */
    static async executeQuery(userId, databaseName, query) {
        // Check if user owns this database
        const owns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, databaseName);
        if (!owns) {
            throw new Error('You do not have permission to access this database');
     ...
     */
    static async testUserConnection(userId) {
        try {
            const userPool = await this.getUserConnectionPool(userId);
     ...
     */
    static async backupDatabase(userId, databaseName, backupPath) {
        // Check if user owns this database
        const owns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, databaseName);
        if (!owns) {
     ...
     */
    static async restoreDatabase(userId, databaseName, backupPath) {
        // Check if user owns this database
        const owns = await user_database_model_1.UserDatabaseModel.userOwnsDatabase(userId, databaseName);
        if (!owns) {
     ...
     */
    static async getBackupHistory(userId, databaseName) {
        if (databaseName) {
